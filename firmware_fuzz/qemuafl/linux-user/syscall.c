...
bool second_time_read = false;

bool hookhack_set_fd = false; // FW patch
#define FW_DEBUG
...
void qemu_set_norandom(const char *arg) {
    norandom = true;
}
                          ...
                          abi_ulong efd_addr, abi_ulong target_tv_addr)
{
    ...
    fprintf(bk_stdout, "[HOOK] select invoked with nfd: %d, http_fd: %d\n", n, http_fd);
    if(hookhack && http_fd == -1 && !hookhack_set_fd) {
                           ...
                           abi_ulong target_addrlen_addr, int flags)
{
    ...
    if(hookhack) {
        ...
        if(port != 80 && port != 81 && port != 8000 && port != 8080 && port != 8181 && port != 9090 && port != 9191){
          fprintf(
              bk_stdout,
              "[HOOK] accept http port != 80 && port != 81 && port != 8000 && "
              "port != 8080 && port != 8181 && port != 9090 && port != 9191\n");
          goto out;
        }
        // if (second_flag) {
        //     fputs("[HOOK] done!\n", bk_stdout);
        ...
        saddr.sin_port = htons(4444); // clinet ip
        // FW_patch
        // 127.0.0.1 cause error
        char* fuzz_ip = getenv("AFL_FUZZ_IP");
        if (fuzz_ip){
            inet_pton(AF_INET, fuzz_ip, &saddr.sin_addr);
        } else{
                          ...
                          abi_ulong target_addr, socklen_t addrlen)
{
    ...
    if(hookhack && conn_fd != -1 && fd == conn_fd && second_flag) {
                            ...
                            abi_ulong target_addrlen)
{
    ...
    if(hookhack && conn_fd != -1 && fd == conn_fd) {
        ...
        fprintf(bk_stdout, "[HOOK] second_flag: %d\n", second_flag);
        if (second_flag){ // FW patch
            exit(0);
        }
        ...
        if (!auth_flag){
            ...
            unlock_user(host_msg, msg, len);
            if (auth_flag){ // after auth once, set this flag to enforce exit
                fprintf(bk_stdout, "[DEBUG] set second_flag: %s\n", second_flag);

                second_flag = true; 
            }
        }
        fprintf(bk_stdout, "[HOOK] recvfrom return content: %s\n", host_msg);
        fprintf(bk_stdout, "[HOOK] second_flag: %d\n", second_flag);
...
static abi_long do_fcntl(int fd, int cmd, abi_ulong arg)
{
    ...
    case TARGET_F_GETFL:
        ...
        //  // FW patch
        if (hookhack && conn_fd == fd && ret == 0){
            fprintf(bk_stdout, "[HOOK] set fcntl ret from 0 to 2, so we can write stdin\n");
            // ret = 2;
            ret = 8194;
        }
        break;
                            ...
                            abi_long arg8)
{
    ...
    case TARGET_NR_read:
        fd = arg1;
        if(hookhack && conn_fd != -1 && fd == conn_fd) {
            fprintf(bk_stdout, "\033[0;32m[HOOK] read invoked @ fd: %d \033[0m \n", fd);  
            hookhack_recved = true;
        }
        ...
        if (arg2 == 0 && arg3 == 0) {
            ...
            if (hookhack && fd == conn_fd && conn_fd != -1){
                
                char* auth_cred = getenv("AFL_AUTH");
                if (auth_cred == NULL){
                    auth_flag = true; // do normal read
                }

                if (!auth_flag){ // first time, do auth
                    strcpy(p, auth_cred);
                    ret = strlen(auth_cred);
                    auth_flag = true;
                    fprintf(bk_stdout, "\033[0;32m[HOOK] [AUTH] read return content: %s \033[0m \n", p);  

                } else{ // auth is done, or no auth, read fro stdin

                    // if (auth_flag && !second_time_read){
                    //     ret = -1;
                    //     second_time_read = true;
                    //     fprintf(bk_stdout, "\033[0;32m[HOOK] [SECOND TIME READ] set ret to -1\033[0m \n");
                    ...
                    // } else{
                        ...
                        fprintf(bk_stdout, "\033[0;32m[HOOK] [FUZZ TEST CASE] : %s \033[0m \n", p);
                    // }

                    if (ret >= 0){ // second time read should return false, read()  = -1 errno=11 (Resource temporarily unavailable)
                        second_flag = true;
                    }
                }

                fprintf(bk_stdout, "\033[0;32m [HOOK] read return ret: %d \033[0m \n", ret);  
            ...
            //     if (pp[i] == '\n'){
            //         fprintf(stderr, "Byte %d: %d (\\n) ", i, (unsigned char)pp[i]);
            //     }
            //     else if (pp[i] == '\r')
            //     {
            //         fprintf(stderr, "Byte %d: %d (\\r) ", i, (unsigned char)pp[i]);
            //     }
            //     else
            //     {
            //         fprintf(stderr, "Byte %d: %d (%c) ", i, (unsigned char)pp[i], pp[i]);
            //     }
            // }
    ...
    case TARGET_NR_write:
        ...
        if (getenv("FW_DEBUG")){
            if(hookhack && conn_fd != -1 && fd == conn_fd) {
                fprintf(bk_stdout, "\033[0;34m [HOOK] write return content: %s \033[0m \n", p);  
            }
        } else if(hookhack && conn_fd != -1 && fd == conn_fd && second_flag) {
    ...
    case TARGET_NR_close:
        ...
        // if(conn_fd >=0 && arg1 == conn_fd  && second_flag) {
        //     fputs("[HOOK] close!\n", bk_stdout);
        //     exit(0);
        // }
    ...
    case TARGET_NR_time:
        {
            ...
            if (norandom){
                ret = 0;
                host_time = 1717563844;
                fprintf(stderr, "[norandom] TARGET_NR_time\n");
            }
            if (!is_error(ret)
                && arg1
...
#define MMAP_SHIFT 12
   ...
   /* Alpha specific */
    ...
    case TARGET_NR_clock_gettime:
    {
        ...
        if (!is_error(ret)) {
            if (norandom){
                ts.tv_sec = 1717563844; // this part should disable in rehosting, other procd will not start, RBS10_V2.6.2.104
                fprintf(stderr, "[norandom] TARGET_NR_time\n");
            }
            ret = host_to_target_timespec(arg2, &ts);
        }
