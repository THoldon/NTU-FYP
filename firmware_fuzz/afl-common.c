...
char **get_qemu_argv(u8 *own_loc, u8 **target_path_p, int argc, char **argv) {
  ...
  int ret = asprintf(&execve_arg, "%s -hackbind -hackproc -norandom", "/qemu-static");
  if (unlikely(ret <= 0)) {
  ...
  new_argv[4] = "-norandom";
  new_argv[5] = "-execve";
  new_argv[6] = execve_arg;
  new_argv[7] = "--";
  new_argv[8] = *target_path_p;
  memcpy(&new_argv[9], &argv[1], (int)(sizeof(char *)) * (argc - 1));
