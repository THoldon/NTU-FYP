...
class WebCheck:
    ...
    def Connect(self, url, auth):
        ...
        options.add_argument('--ignore-certificate-errors') # qc add
        options.add_argument('--disable-dev-shm-usage') # qc add
        options.set_capability('unhandledPromptBehaviour', 'dismiss')
        options.set_capability('unexpectedAlertBehaviour', 'dismiss')
        self.driver = webdriver.Chrome(options=options)
        self.driver.set_page_load_timeout(180) # qc add
        try:
            if ":" in auth:
                index = url.index("://")
                splitIndex = index+3
                ...
                # # qc add debug # UPDATE: cant set this :Message: unexpected alert open:
                # final_url = self.driver.current_url
                # if final_url != head + auth + '@' + tail:
                #     print(f"Redirected from {head + auth + '@' + tail} to {final_url}")
                ...
                try:
                    ...
                    if "<html" in page_source or "<script" in page_source:
                        ...
                        if "401" in raw_data.lower() and "unauthorized" in raw_data.lower():
                            if "enabling CAPTCHA Graphical Authentication provides added security protection to prevent unauthorized".lower() not in raw_data.lower():  # qc add, it works normally
...
class Login:
    ...
    def check_login_type(ip, brand):
        ...
        if r.status_code == 401:
            ...
            if brand == "dlink":  # dlink normal login
                ...
                return 'unknown'
        elif r.text.find('<div class="message">Login to the router : </div>') != -1: # DIR_600_REVB_FIRMWARE_PATCH_v2.18_J1MD, qc add
            return 'dlink_600'
            ...
            if r.text.find('/login/Auth') != -1:
                ...
                if r and r.status_code == 200:
                    if r.text == "1" or r.text == '0':  # qc add
    ...
    def login(session, brand, ip, login_type, username, password):
        ...
        if login_type == 'basic':
        ...
        elif login_type == 'dlink_600': # qc add, dlink dir 600
        ...
        elif login_type == 'dlink_dap2360': # qc add, dlink dir 600
            login_cgi = "login.php"
            login_name = username
            log_pass = password
            ...
            # # qc tmp
            # req = requests.Request('POST', ip+'/HNAP1/', headers=headers, data=payload, cookies=cookies)
            #
            # # 准备请求
...
class Initializer:
    def __init__(self, brand, ip, port, username, password):
        ...
        if self.brand == "netgear":
            ...
            self.pattern = re.compile(r'(<div id="wizard_title">Welcome</div>|Welcome to the D-Link Setup Wizard)') # qc add, for fuzzing, irrelevant with http checker
            ...
            try:
                ...
                if self.brand == "netgear":
                    ...
                    if "PAGE.OnClickCancel()" in page_source: # DIR 845
                        button = self.driver.find_element(By.NAME, 'b_exit')  # qc add
                        button.click()
    ...
    def ClickNext(self):
        if self.driver.page_source.find('btnsContainer_div') != -1: # wnr2000v3, WNDR3800, JNR3210, R6200v2
            ...
            if isinstance(self.driver.find_elements(By.ID, 'btnsContainer_div'), list): # qc add, JNR3210_V1.1.0.30_1.0.2
